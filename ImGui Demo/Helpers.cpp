...
{
    auto rc = rainbowColor(getCurrentTime(), color.rainbowSpeed, color.color[3]);
    color.color[3] *= alphaFactor;
    return ImGui::ColorConvertFloat4ToU32(color.rainbow ? ImVec4{ rc[0], rc[1], rc[2], 1.0f } : ImVec4{ color.color[0], color.color[1], color.color[2], 1.0f });
}

unsigned int Helpers::calculateColor(Color3 color) noexcept
{
    auto rc = rainbowColor(getCurrentTime(), color.rainbowSpeed, 1.0f);
    return ImGui::ColorConvertFloat4ToU32(color.rainbow ? ImVec4 { rc[0], rc[1], rc[2], 1.0f } : ImVec4{ color.color[0], color.color[1], color.color[2], 1.0f});
}
...
float Helpers::getCurrentTime() noexcept
{
...
void Helpers::rainbowMenuBorder(float speed) noexcept
{
    ...
    auto rc = rainbowColor(getCurrentTime(), speed * 10, 1.0f);
    colors[ImGuiCol_Border] = ImVec4{ rc[0],rc[1] ,rc[2] , 1.0f};
}
